"""
The architecture of the VNECT is defined, the weights from checkpoint files are taken as input 
and the network is initialized with these weights
"""
# Reference:
...
class VNect:
    ...
    def load_weights(self, sess, pkl_file):
        vnect_weights = pickle.load(open(pkl_file, 'rb'))
        with tf.variable_scope('', reuse=True):
            for variable in tf.global_variables():
                var_name = variable.name.split(':')[0]
                self.assign_weights_from_dict(var_name, vnect_weights, sess)

    @staticmethod
    def assign_weights_from_dict(var_name, vnect_weights, sess):
        with tf.variable_scope('', reuse=True):
            var_tf = tf.get_variable(var_name)
            print('assigning', var_tf)
            sess.run(tf.assign(var_tf, vnect_weights[var_name]))
            np.testing.assert_allclose(var_tf.eval(sess), vnect_weights[var_name])


if __name__ == '__main__':
    model = VNect()
    print('The network was built successfully.')